            ...
            <LoginDiv>
                <LoginForm>
                    ...
                    <SignInForm onSubmit={formik.handleSubmit}>
                        <SignInput placeholder={t('username')} id="userName"
                            name="userName"
                            type="text"
                            onChange={formik.handleChange}
                            value={formik.values.userName} />
                        {formik.errors.userName && <ErrorText>{formik.errors.userName}</ErrorText>}
                        <SignInput placeholder={t('password')} id="password"
                            name="password"
                            type="password"
                            onChange={formik.handleChange}
                    ...
                    <DropdownLang isOpen={isOpen} toggle={toggle}>
                        ...
                        <DropdownMenuLang>
                            {Object.keys(lngs).map((lng) => (
                                <DropdownItem type='submit' className={i18n.resolvedLanguage === lng ? 'd-none' : ''} key={lng} onClick={() => i18n.changeLanguage(lng)}><img src={flags[lng]} alt={flags[lng]} /></DropdownItem>
                            ))}
                        </DropdownMenuLang>
                    </DropdownLang>
                </LoginImage>
